...
class SingleHypothesisTracker:
    ...
    def __init__(self,
        ...
        self.filter = filter(spacecraft = agent,
                             mean = mean,
    ...
    def run(self,observations:np.ndarray,times):
        ...
        #data association 
        most_likely_measurement, missed_detections[0] = self.data_association(observation=observations[:,:,0])
        ...
        #time loop
        loop_iterable = list(zip(times[1:], times[:-1], observations.transpose(2,0,1)[1:]))
        for i, (tk, tkm1, observation) in enumerate(tqdm(loop_iterable, desc="Running SHT Filter", unit="step")):

            ##propagation
            self.time_update(duration=tk - tkm1)

            ##date association
            most_likely_measurement, missed_detections[i+1] = self.data_association(observation=observation)
    ...
    def data_association(self, observation:np.ndarray):
        ...
        measurement_mean, measurement_cov = sigma_points.measurement_map(sensor=self.sensor)
        
        hypotheses_array = np.array(
            [
                mahalanobis_distance(measurement_mean, meas[:, np.newaxis], measurement_cov)[0, 0] ** 2
        ...
        if most_likely_measurement:
            return (observation[:, indices[most_likely_measurement[0]][0]], 0)
    ...
    def measurement_update(self, measurement:np.ndarray,time):
        ...
        sigma_points = SigmaPoints(self.state.mean, self.state.covariance)
        state_estimate,covariance_estimate,measurement_mean,measurement_cov =  self.filter.measurement_update(measurement,
                                       sigma_points,
        ...
        self.data_handler.add_measurement(measurement_mean=measurement_mean,measurement_covariance=measurement_cov,time=time)
    ...
    def time_update(self,duration: float):
        ...
        sigma_points = self.filter.time_update(state_estimate = self.state.mean,
