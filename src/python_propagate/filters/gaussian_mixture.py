...
class GaussianMixture:
    ...
    def run(self, measurements, times):
        ...
        filter_state = self.measurement_update(
            measurement=measurements[0, :].T, time=times[0]
        )
        # self.data_handler.add_state(copy(filter_state), time=times[0])

        # store state data
        self.data_handler.add_state(filter_state, time=times[0])

        self.component_hist.append(len(self.states))
        self.merge_prune_truncate()

        for i, (tk, tkm1, measurement) in enumerate(
            zip(times[1:], times[0:-1], measurements[1:])
        ):
    ...
    def measurement_update(self, measurement, time):
        ...
        for i, (state) in enumerate(self.states):
            ...
            (
                ...
                measurement, sigma_points, state.state_mean, state.state_covariance
            )
            ...
            q_measurement = multivariate_normal.pdf(
                x=measurement.ravel(),
                mean=measurement_mean.ravel(),
                cov=measurement_covariance,
            )
    ...
    def merge_prune_truncate(self, j_max=800):
        ...
        while subset_i:
            ...
            mean_l = np.sum(
                weight / weight_l * state.state_mean for state, weight in subset_l
            )
            covariance_l = np.sum(
                weight
                / weight_l
                * (
                    ...
                    + np.outer(state.state_mean, state.state_mean)
                )
                for state, weight in subset_l
            ) - np.outer(mean_l, mean_l)
    ...
    def merge_distribution(self):
        ...
        covs = np.array(
            [state.state_covariance for state in self.states]
        )  # .transpose(1,2,0)
        ...
        p_temp = np.array(
            [
                cov
                + np.outer(
                    x_l[:, np.newaxis] - state_bar, x_l[:, np.newaxis] - state_bar
                )
                for x_l, cov in zip(means, covs)
            ]
        )
