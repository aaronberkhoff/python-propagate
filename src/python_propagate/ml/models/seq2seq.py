...
from python_propagate.ml.models.lstm import BayesianLSTM, BayesianLinear, BayesianLSTMCell
...
class BayesianDecoder(nn.Module):
    ...
    def forward(self, x, h, c):
        ...
        output = self.fc(h_flat)  # Shape after fc: (batch_size * seq_len, output_size)
...
class Seq2Seq(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, device='cuda'):
        ...
        self.decoder = BayesianDecoder(input_size, hidden_size, output_size, prior_std=.01)
    ...
    def train_model(self, train_loader, n_epochs, scaler, lr=0.001):
        ...
        # Ensure no parameter is used in more than one group
        optimizer_params = [
            {'params': model_params, 'lr': lr},  # The parameters of the model (encoder and decoder)
            {'params': self.log_vars.parameters(), 'lr': 1e-3}  # Log variance parameters with a different learning rate
        ]
        ...
        for epoch in range(n_epochs):
            ...
            for x, y in train_loader:
                ...
                #Baysian requirement:
                kl_loss = self.encoder.kl_loss() + self.decoder.kl_loss()
                

                # Compute individual losses
                mse_loss = criterion(output, y) * 1.0
                energy1 = energy_difference_loss(output, y, scaler, weight=1.0)
                energy2 = energy_loss(output, scaler, weight=1.0)
                # sym_loss = symplectic_loss(output, y, scaler, dt=30, weight=1.0)
                sym_loss = symplectic_loss(output, y, scaler, dt=3600, weight=1.0)
                pin_loss = pinn_loss(output, y, scaler, weight=1.0)

                # Normalize losses dynamically using log variance
                mse_loss_normalized = torch.exp(-self.log_vars['mse']) * mse_loss + self.log_vars['mse']
                energy1_normalized = torch.exp(-self.log_vars['energy1']) * energy1 + self.log_vars['energy1']
                energy2_normalized = torch.exp(-self.log_vars['energy2']) * energy2 + self.log_vars['energy2']
                sym_loss_normalized = torch.exp(-self.log_vars['sym']) * sym_loss + self.log_vars['sym']
                pin_loss_normalized = torch.exp(-self.log_vars['pinn']) * pin_loss + self.log_vars['pinn']
            ...
            norm_mse = torch.exp(-self.log_vars['mse']).item()
            norm_energy1 = torch.exp(-self.log_vars['energy1']).item()
            norm_energy2 = torch.exp(-self.log_vars['energy2']).item()
            norm_sym = torch.exp(-self.log_vars['sym']).item()
            norm_pinn = torch.exp(-self.log_vars['pinn']).item()

            norm_total = norm_mse + norm_energy1 + norm_energy2 + norm_sym + norm_pinn

            scheduler.step(norm_total)

            # Print nicely formatted summary of all losses
            # Clear last 2 printed lines to refresh output
            sys.stdout.write("\033[F" * 2 + "\r")
            sys.stdout.flush()
            ...
            print(
                f"  Normalized MSE: {norm_mse:.3e} | "
                f"Energy Diff: {norm_energy1:.3e} | "
                f"Energy: {norm_energy2:.3e} | "
                f"Symplectic: {norm_sym:.3e} | "
                f"PINN: {norm_pinn:.3e} | "
                f"Norm Total: {norm_total:.3e}"
            )
