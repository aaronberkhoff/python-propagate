...
class ProperElements():
    ...
    def __init__(self, orbital_element_data: Iterable[OrbitalElements], duration, dt = 30):
        ...
        self.mean_orbital_elements_data = [butter_lowpass_filter(data=oe,cutoff_freq=1/orbital_period, fs = fs) for oe in oe_data]
        self.mean_element_segments = [split_list(oe,segment) for oe in self.mean_orbital_elements_data]

        oe_data = []
        for oe in self.mean_element_segments:
            segment_data = []    
            for seg in oe:
                segment_data.extend(fft_lowpass_filter(data=seg,cutoff_freq=cutoff_freq/segment,fs=fs))
