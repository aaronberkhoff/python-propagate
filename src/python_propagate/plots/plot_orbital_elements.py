...
def plot_orbital_elements(
    agents, scenario, output_directory, name="orbital_elements", legend=True, save=True
):
    """
    ...
    axes = (
        axes.flatten()
    )  # for easier indexing: 0:SMA, 1:ecc, 2:inc, 3:RAAN, 4:Arg Periapsis, 5:true anomaly
    ...
    for agent in agents:
        ...
        orbital_element_data = [
            state.to_keplerian(scenario.central_body.mu) for state in agent.state_data
        ]
