...
def plot_residuals(
    ...
    if plot_type == "position":
        ...
        times = np.array(data_handler.state_times)
        ...
        times = np.array(data_handler.measurement_times)
        ...
        cov = np.array(data_handler.measurement_covariances)
        res = np.array([res.ravel() for res in data_handler.residuals])
        times = np.array(data_handler.merged_times)
        labels = ["RA [DEG]", "DEC [DEG]"]
        title = "Merged Measurement Residuals"
        index = 0
    else:
        raise ValueError(
            f"Invalid plot_type <{plot_type}>. Check spelling or support plot types"
        )
    ...
    for i, (ax, label) in enumerate(zip(axs, labels)):
        plot_single_residual(
            ax, res[:, i + index], cov[:, i + index, i + index], times, label, ylim=ylim
        )
