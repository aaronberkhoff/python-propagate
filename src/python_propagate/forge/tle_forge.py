...
class TLEForge(Forge):
    ...
    def __init__(
        ...
        plots=None,
        name="TLEForge",
        forge: str = "AstroForge",
    ):

        Genes = namedtuple(
            "Genes", ["agents"]
        )  # Create a namedtuple for genes, as TLEForge needs agents to be passed in.
        self.agent_base = agent_base  # Store the agent base for TLEForge, used to extract agents from the database.
        genes = Genes(
            agents=self._get_agents_from_database(
                database=database, norad_ids=norad_ids, scenario=scenario
            )
        )  # Extract agents from the database.
        ...
        if forge == "AstroForge":
            ...
            raise ValueError(
                f"Invalid forge type '{forge}' specified. Use 'AstroForge' or 'PhotoForge'."
            )
    ...
    def _get_agents_from_database(self, database, norad_ids, scenario):
        ...
        for norad_id in norad_ids:
            cursor.execute(
                f"SELECT * FROM {tables[0][0]} WHERE norad_cat_id = {norad_id}"
            )  # Adjust 'tle_data' and 'norad_id' to your table/column names
            ...
            for i, row in enumerate(rows):
                ...
                if scenario.duration > (
                    datetime.strptime(row[11], DATESTR)
                    - datetime.strptime(rows[0][11], DATESTR)
                ):
                    ...
                    state = State(
                        position=np.array(radius),  # Position in km
                    ...
                    if i == len(rows) - 1:
                        ...
                        duration = (
                            datetime.strptime(rows[i + 1][11], DATESTR)
                            - agent.start_time
                        )  # Assuming row[1] is the epoch time in the database, adjust as needed
                        ...
                        times = [
                            state.time for state in agent.state_data
                        ]  # Get the times of the propagated states
            ...
            print(
                f"Extracted agent for NORAD ID {norad_id}: {agent.name} with {len(agent.state_data)} states."
            )  # Debugging output
            pass

        return agents

    def get_agents_from_database_parallel(
        self, database, norad_ids, scenario, max_workers=4
    ):
        """
        ...
        args_list = [
            (database, deepcopy(self.agent_base), norad_id, scenario)
            for norad_id in norad_ids
        ]
    ...
    def process_agent(
        self, agent, scenario, datatypes, add_noise=False, propagate=False
    ):
        return self.parent_class.process_agent(
            self, agent, scenario, datatypes, add_noise, propagate
        )

    def get_orientation_history_from_manuever(self, agent, time_data):

        try:
            return self.parent_class.get_orientation_history_from_manuever(
    ...
    def generate_data_parallel(self, cores):
        ...
        print(
            "WARNING: Parallel is not supported for TLE forge right now. Reverting to serial mode."
        )
    ...
    for i, row in enumerate(rows):
        if scenario.duration > (
            ...
            if i == len(rows) - 1:
                ...
                agent.start_time = datetime.strptime(row[11], DATESTR)
                duration = (
                    datetime.strptime(rows[i + 1][11], DATESTR) - agent.start_time
                )
