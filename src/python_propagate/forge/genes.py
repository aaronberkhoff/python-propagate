...
def set_subattribute(agent, public_attrs, attribute, value, name_index=None):
    ...
    if not set_bool:
        print(f"<{attribute}> is not a settable attribute of {agent.name}")
...
def get_subattribute(agent, public_attrs, attribute):
    ...
    """
    results = (
        None  # Initialize the return value to None, in case no attribute is found.
    )
    for att in public_attrs:
        try:
...
class Gene:
    ...
    def __init__(self, agent_base: Agent, attribute: str, samples: int, **kwargs):
        ...
        for i, value in enumerate(values):
            ...
            if not hasattr(
                agent_base, attribute
            ):  # If the base agent doesn't have the attribute
                set_subattribute(
                    agent=agent,
...
def combine(genes: Iterable):
    ...
    public_attrs = [
        attr for attr in dir(genes[0].agent_base) if not attr.startswith("_")
    ]
