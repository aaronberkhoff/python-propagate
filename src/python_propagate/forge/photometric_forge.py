...
class PhotoForge(Forge):
    ...
    ):  # TODO This needs to be changed like crazy
        orientation_data = []
    ...
    def process_agent(
        ...
        data_agent = []
        state_data = []
        orientation_data = self.get_orientation_history_from_manuever(
            agent,
            [i * agent.dt.total_seconds() for i, _ in enumerate(agent.state_data)],
        )
        for i, (state, orientation) in enumerate(
            zip(agent.state_data, orientation_data)
        ):
            ...
            for station in scenario.stations:
                ...
                flux_received, apparent_magnitude, is_visible = (
                    station.calculate_light_flux(state=state, agent=agent)
                )

                state.metadata["flux_w_m2"] = (
                    flux_received  # Preserve any existing metadata in the state object, if present.
                )
                state.metadata["apparent_magnitude"] = (
                    apparent_magnitude  # Store the apparent magnitude in the state metadata for reference.
                )
                state.metadata["orientation"] = orientation

                state_data.append(
                    deepcopy(state)
                )  # TODO: SOmething is wrong here. I should Not have to do a deep copy
